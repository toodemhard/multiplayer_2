...
void local_scene_init(LocalScene* scene, Arena* level_arena, Input::Input* input, Renderer* renderer, Slice<Font> fonts) {
    *scene = {};
    ...
    scene->player_handle = create_player(&scene->state);
...
void local_scene_update(LocalScene* s, Arena* frame_arena, double delta_time) {
    ...
    if (s->input->mouse_up(SDL_BUTTON_LEFT)) {
        next_frame = true;
    }
    ...
    while (s->accumulator >= fixed_dt) {
        ...
        s->inputs[0] = input_state(s->tick_input, s->camera);
        if (s->move_submit) {
            s-> move_submit = false;
            s->inputs[0].move_spell_src = s->spell_move_src;
            s->inputs[0].move_spell_dst = s->spell_move_dst;
