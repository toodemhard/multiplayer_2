...
glm::vec2 f32x2_to_vec2(f32 vec[2]) {
    return {vec[0], vec[1]};
}
...
// bool button(UI* ui, const char* label, ElementProps props) {
//     
//
// }
...
//     );
//
//     auto scale = font::text_dimensions(*m_default_font, text,style.font_size);
//
//     // auto max_width = [&]() {
...
// }
//
// void UI::add_parent_scale(Row& parent_row, glm::vec2 scale) {
//     auto& rect = m_rects[parent_row.rect_index];
//
//     auto length_axis = [&](const glm::vec2& scale) {
...
//     };
//
//     auto orthogonal_length = [&](const glm::vec2& scale) {
//         switch (parent_row.style.stack_direction) {
//         case StackDirection::Horizontal:
//             return scale.y;
//             break;
...
// }
//
// float aligned_axis(const glm::vec2& scale, const StackDirection& stack_direction) {
...
//         add_parent_scale(m_rows[m_row_stack.back()], glm::vec2{width, height});
//     }
//
...
// };
//
// bool rect_point_intersect(glm::vec2 point, glm::vec2 position, glm::vec2 scale) {
//     const glm::vec2& p1 = position;
//     const glm::vec2 p2 = position + scale;
//
...
//         auto g_st = Style{};
//         g_st.position = Position::Absolute{hook.position + glm::vec2{0, hook.scale.y}};
...
//
//     auto incr = [](RowStackFrame& parent, glm::vec2 scale, float gap) {
...
//                 new_row.next_position = glm::vec2{current_row.style.padding.left};
//             } else {
...
//                 text.position = row_stack.back().next_position + glm::vec2{(parent_rect.scale.x - x_padding - font::text_dimensions(*m_default_font, text.text, text.font_size).x) / 2.0f, 0.0f};
//             break;
//             }
//     
...
// }
//
// SDL_FPoint vec2_to_sdl_fpoint(const glm::vec2& vec) {
//     return {vec.x, vec.y};
// }
//
...
// //     const glm::vec2& scale = {rect.w, rect.h};
...
//             font::draw_text(renderer, *m_default_font, text.text, text.font_size, text.position, text.text_color, text.max_width);
//
//             text_index++;
//
//         }
