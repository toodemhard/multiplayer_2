...
{
    Renderer*           Renderer;       // Main viewport's renderer
    Texture            FontTexture;
...
// Functions
bool ImGui_ImplSDLRenderer3_Init(Renderer* renderer)
{
    ImGuiIO& io = ImGui::GetIO();
    IMGUI_CHECKVERSION();
    IM_ASSERT(io.BackendRendererUserData == nullptr && "Already initialized a renderer backend!");
    IM_ASSERT(renderer != nullptr && "Renderer not initialized!");
...
static void ImGui_ImplSDLRenderer3_SetupRenderState(Renderer* renderer)
{
...
// static int SDL_RenderGeometryRaw8BitColor(Renderer* renderer, ImVector<SDL_FColor>& colors_out, SDL_Texture* texture, const float* xy, int xy_stride, const SDL_Color* color, int color_stride, const float* uv, int uv_stride, int num_vertices, const void* indices, int num_indices, int size_indices)
// {
...
void ImGui_ImplSDLRenderer3_RenderDrawData(ImDrawData* draw_data, Renderer* renderer)
{
    ImGui_ImplSDLRenderer3_Data* bd = ImGui_ImplSDLRenderer3_GetBackendData();
    ...
    //             bd->FontTexture
                ...
                // for (int i = 0; i < num_vertices; i++) {
                //     auto& vtx = vertices[i];
                //     vtx.position.x /= (float)renderer->window_width;
                //     vtx.position.y /= (float)renderer->window_height;
                // }

                renderer::render_geometry_textured(*renderer, bd->FontTexture, vertices.data(), num_vertices, idx_buffer + pcmd->IdxOffset, pcmd->ElemCount, sizeof(ImDrawIdx));
            }
        }

        // renderer::draw_textured_rect(*renderer, {}, Rect{{200,200},{500,500}}, bd->FontTexture);
    }
