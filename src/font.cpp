...
void save_font_atlas_image() {
    ...
    generate_text_font_atlas(bitmap);
    auto& config = text_font_atlas_config;

    std::cout << stbi_write_png("font_atlas.png", config.width, config.height, 1, bitmap.data(), 1 * config.width) << "\n";
}
//
// void save_icons() {
...
void draw_text(SDL_Renderer* renderer, const char* text, float font_size, glm::vec2 position, RGBA color, float max_width) {
    ...
    while (*text && x - position.x <= max_width) {
        ...
        if (c == '\n') {
            ...
            auto texture = text_font_atlas.texture;
            stbtt_packedchar char_info = text_font_atlas.char_data[c - 32];
